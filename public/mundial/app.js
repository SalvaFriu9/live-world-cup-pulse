...
function matchCard(f) {
  ...
  return `
    <div class="match-card ${live ? "live" : ""}" data-mid="${f.fixture.id}">
      <div class="mc-head">
        ${statusHtml}
...
async function openMatchModal(id) {
  ...
  $("#modal-body").innerHTML = `
    <h2 style="margin-top:0">${f.teams.home.name} ${fmt(f.goals.home)} - ${fmt(f.goals.away)} ${f.teams.away.name}</h2>
...
function renderStatsModal(stats, f) {
  ...
  if (!stats || stats.length < 2) {
    el.innerHTML = `<p style="color:var(--muted);text-align:center;padding:20px">
      ${API_KEY ? "Estadísticas no disponibles para este partido." : "Activa tu API_KEY para ver estadísticas en vivo (posesión, tiros, xG, tarjetas, etc.)."}
    </p>`;
    return;
  }
