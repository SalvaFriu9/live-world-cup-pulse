...
/* ---------- DATOS DEMO (fallback sin API key) ---------- */
const DEMO = {
fixtures: [
{
fixture: { id: 1, date: new Date(Date.now() + 3600e3).toISOString(), status: { short: "NS", elapsed: null }, venue: { name: "MetLife Stadium", city: "Nueva Jersey" }, referee: "Szymon Marciniak" },
league: { round: "Group A - 1" },
teams: { home: { name: "México", logo: flagUrl("mx") }, away: { name: "Canadá", logo: flagUrl("ca") } },
goals: { home: null, away: null },
},
{
fixture: { id: 2, date: new Date().toISOString(), status: { short: "1H", elapsed: 23 }, venue: { name: "SoFi Stadium", city: "Los Ángeles" }, referee: "Anthony Taylor" },
league: { round: "Group B - 1" },
teams: { home: { name: "Argentina", logo: flagUrl("ar") }, away: { name: "Brasil", logo: flagUrl("br") } },
goals: { home: 1, away: 0 },
},
{
fixture: { id: 3, date: new Date().toISOString(), status: { short: "2H", elapsed: 67 }, venue: { name: "Estadio Azteca", city: "CDMX" }, referee: "Daniele Orsato" },
league: { round: "Group C - 1" },
teams: { home: { name: "España", logo: flagUrl("es") }, away: { name: "Francia", logo: flagUrl("fr") } },
goals: { home: 2, away: 2 },
},
{
fixture: { id: 4, date: new Date(Date.now() - 86400e3).toISOString(), status: { short: "FT", elapsed: 90 }, venue: { name: "BC Place", city: "Vancouver" }, referee: "Clément Turpin" },
league: { round: "Group D - 1" },
teams: { home: { name: "Inglaterra", logo: flagUrl("gb") }, away: { name: "Alemania", logo: flagUrl("de") } },
goals: { home: 3, away: 1 },
},
{
fixture: { id: 5, date: new Date(Date.now() + 2 * 86400e3).toISOString(), status: { short: "NS", elapsed: null }, venue: { name: "AT&T Stadium", city: "Dallas" }, referee: "Por designar" },
league: { round: "Group E - 1" },
teams: { home: { name: "Portugal", logo: flagUrl("pt") }, away: { name: "Uruguay", logo: flagUrl("uy") } },
goals: { home: null, away: null },
},
{
fixture: { id: 6, date: new Date().toISOString(), status: { short: "HT", elapsed: 45 }, venue: { name: "Mercedes-Benz Stadium", city: "Atlanta" }, referee: "Facundo Tello" },
league: { round: "Group F - 1" },
teams: { home: { name: "Países Bajos", logo: flagUrl("nl") }, away: { name: "Italia", logo: flagUrl("it") } },
goals: { home: 1, away: 1 },
},
],
...
return   <div class="match-card ${live ? "live" : ""}" data-mid="${f.fixture.id}">   <div class="mc-head">   ${statusHtml}   ${group ?<span class="mc-group">${group}</span>: ""}   </div>   <div class="mc-teams">   <div class="mc-team">   <img src="${f.teams.home.logo}" alt="${f.teams.home.name}" loading="lazy" />   <div class="name">${f.teams.home.name}</div>   </div>   <div class="mc-score">${score}</div>   <div class="mc-team">   <img src="${f.teams.away.logo}" alt="${f.teams.away.name}" loading="lazy" />   <div class="name">${f.teams.away.name}</div>   </div>   </div>   <div class="mc-foot">   <span>📅 ${formatDate(f.fixture.date)}</span>   <span>🏟️ ${f.fixture.venue?.name || "—"}</span>   ${f.fixture.venue?.city ?<span>📍 ${f.fixture.venue.city}</span>: ""}   ${f.fixture.referee ?<span>🧑‍⚖️ ${f.fixture.referee}</span>: ""}   </div>   </div>  ;
}
...
// standings es array de grupos (cada grupo = array de teams)
...
const rows = grp.map(t =>    <tr class="${t.rank <= 2 ? "qualified" : ""}">   <td class="team"><img src="${t.team.logo}" alt=""/>${t.team.name}</td>   <td>${t.all.played}</td>   <td>${t.all.win}</td>   <td>${t.all.draw}</td>   <td>${t.all.lose}</td>   <td>${t.all.goals.for}:${t.all.goals.against}</td>   <td>${t.goalsDiff > 0 ? "+" : ""}${t.goalsDiff}</td>   <td><strong>${t.points}</strong></td>   </tr>).join("");
return <div class="group-card">   <div class="group-head">${groupName}</div>   <table class="group-table">   <thead><tr><th style="text-align:left">Equipo</th><th>PJ</th><th>G</th><th>E</th><th>P</th><th>GF:GC</th><th>DG</th><th>Pts</th></tr></thead>   <tbody>${rows}</tbody>   </table>   </div>;
}).join("");
}
...
return <div class="scorer-row">   <div class="scorer-rank">${i + 1}</div>   <div class="scorer-info">   <div class="name">${p.player.name}</div>   <div class="team"><img src="${st.team.logo}" alt=""/>${st.team.name}</div>   </div>   <div class="scorer-goals">${st.goals.total ?? 0}<small>${st.goals.assists ?? 0} asistencias</small></div>   </div>;
}).join("");
}
...
return    <div class="stats-row">   <div style="text-align:right">   <div class="stat-num">${s.value ?? 0}</div>   <div class="stat-bar right"><div class="stat-bar-fill" style="width:${(hVal/total)*100}%"></div></div>   </div>   <div class="stat-label-center">${labels[s.type] || s.type}</div>   <div>   <div class="stat-num">${a}</div>   <div class="stat-bar"><div class="stat-bar-fill" style="width:${(aVal/total)*100}%"></div></div>   </div>   </div>;
}).join("");
el.innerHTML = rows;
}
